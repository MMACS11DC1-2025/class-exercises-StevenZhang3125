...
"""
PLAN
1. Find asphalt colour
2. Identify parking regions
3. Find amount of non-asphalt colours
4. Estimate open parking spots based on colour ratios
"""
...
def whiteLinesFinder(file, start):
    ...
    for col in range(start[0], w, stepW):
        for row in range(start[1], h, stepH):
            r, g, b = pixels[col,row]
            avgRange = (200, 255) # White colour range/tolerance
            ...
            if isWhite:
                if (col, row) not in allVisited: # Skip already visited pixels
                    ...
                    if len(visited) < 50:            # Ignore small white areas (noise)
                        continue
    ...
    if not allVisited:                                                 # No white pixels found
        return whiteLinesFinder(file, (stepW//2, stepH//2)) # Retry with offset start point (half of previous step)
    return whiteLinesRaw                                               # Return partitioned white lines (Segments; edges intercept other segments) (Possibly deprecated)
...
def bfs(file, start, colours):
    ...
    while queue:
        ...
        if colours == "white":    # Custom white range
            colourRange = [(185, 255), (185, 255), (185, 255)]
        else:
