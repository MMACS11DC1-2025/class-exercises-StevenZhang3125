...
This project analyzes solar irradiance maps from the [Global Solar Atlas](https://globalsolaratlas.info/map) to determine the best locations for solar panel installations. The program processes 10 color-coded photovoltaic power output maps representing different geographical locations, calculating the energy generation potential (in kWh/kWp) and consistency of each region.
...
3. The program calculates the sum of absolute differences between each individual pixel color value and the reference color values (Manhattan distance)
...
- **Distance-Based Matching:** Using Manhattan distance for RGB comparison effectively handles minor color variations due to compression or display differences
...
### Edge Cases Handled
- Missing image files (try-except blocks prevent crashes)
- Invalid user input for kW values (defaults to 1kW)
- Invalid menu selections (input validation with error messages)
...
Location 1; Image 1
-----------------------------------------------------------------
You'll average 1547.32 kWh for the region in image 1.
The consistency of the region's kWh is 23.45%
Image 1 processing took 1.234s

Total processing time for all images: 12.567s
```
...
2. **Color Matching (`getClosestValue()`):** For each non-grayscale pixel, the program compares against 19 reference colors, performing RGB distance calculations. This adds additional computational consumption.
...
- Vectorized operations using NumPy could speed up pixel processing significantly
- Binary Search could optimize the color reference lookup

## Challenges Faced

### Challenge 1: UI Element Contamination
...
**Solution:** Implemented the `isGrayScale()` function to detect and filter neutral-colored pixels (where R, G, B values are $\pm 10$ units of the average). This effectively removes UI elements while preserving actual colored data.
